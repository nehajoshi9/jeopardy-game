...
const GameBoard = ({ onClueClick, onHover, clickedClues, rc, questions, answers }) => {
                        ...
                        <div
                            ...
                            onClick={(e) => {
                                if (!clickedClues.has(`${rc[colIndex]}-${(rowIndex + 1) * 200}`)) {
                                    onClueClick(rc[colIndex], rowIndex, value, questions[colIndex][rowIndex], answers[colIndex][rowIndex]);
                                }
                            }}
                            onMouseOver={() => {
                                if (!clickedClues.has(`${rc[colIndex]}-${(rowIndex + 1) * 200}`)) {
